-- Script para aplicar migrações de forma segura no Supabase
-- Este script lida com potenciais erros e evita quebrar o banco de dados

-- Iniciar uma transação para garantir que todas as alterações sejam aplicadas como uma unidade
BEGIN;

-- Verificar se a tabela de controle de versão existe, senão criar
...
DO $$
DECLARE
    migration_name VARCHAR := '20230808_create_ecommerce_settings';
    migration_exists BOOLEAN;
BEGIN
    SELECT EXISTS (
        SELECT 1 FROM migration_versions WHERE name = migration_name
    ) INTO migration_exists;
    
    IF migration_exists THEN
        RAISE NOTICE 'Migração % já foi aplicada. Ignorando...', migration_name;
    ELSE
        RAISE NOTICE 'Aplicando migração %...', migration_name;
        ...
        INSERT INTO migration_versions (name) VALUES (migration_name);
        RAISE NOTICE 'Migração % aplicada com sucesso!', migration_name;
    END IF;
END $$;

-- Executar em um bloco separado para criar usuário admin
DO $$
DECLARE
  admin_id UUID;
BEGIN
  -- Obter o ID do primeiro usuário ou criar um se não houver nenhum
  SELECT id INTO admin_id FROM auth.users LIMIT 1;
  
  IF admin_id IS NULL THEN
    RAISE NOTICE 'Nenhum usuário encontrado. Crie um usuário antes de atribuir papéis.';
  ELSE
    -- Inserir o papel de admin para o primeiro usuário
    INSERT INTO user_roles (user_id, role)
    VALUES (admin_id, 'admin')
    ON CONFLICT (user_id, role) DO NOTHING;
    
    RAISE NOTICE 'Papel de admin atribuído ao usuário %', admin_id;
  END IF;
END $$;
